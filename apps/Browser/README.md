...
- 提供 `/usr/sbin/media-browser-ota` 作为设备无关的 A/B 升级工具。执行前必须由产品
  配置 `OTA_SLOT_A`、`OTA_SLOT_B` 和可选的 `OTA_ACTIVE_FILE`；支持 SHA-256 校验，
  `OTA_DRY_RUN=1` 可只验证目标槽位而不写盘。该工具不会猜测 RK3506 分区布局。
...
| `deploy/*` | BusyBox init、USB/SD 挂载、OTA、systemd、默认运行参数和统一启动 wrapper |
