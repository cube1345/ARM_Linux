 ...
 */
static int is_audio(enum file_type type)
{
 ...
 * @return 成功返回 0，失败返回 -1。
 */
static int load_selected_image(struct browser_app *app)
{
 ...
 * @return 成功返回 0，失败返回 -1。
 */
static int render_text_page(struct browser_app *app)
{
 ...
 */
static int handle_file_key(struct browser_app *app, enum input_action action)
{
 ...
 * @return 继续返回 0，退出返回 1，失败返回 -1。
 */
static int handle_file_touch(struct browser_app *app,
